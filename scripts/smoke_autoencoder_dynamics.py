# Run exact CPU resume and full-epoch gates after the autoencoder cache is frozen.
# The downstream architecture, action cache, roles, and validation policy remain locked.
import json
...
output = Path(settings["output_directory"] + "_dynamics")
uninterrupted_output = Path(settings["uninterrupted_output_directory"] + "_dynamics")
