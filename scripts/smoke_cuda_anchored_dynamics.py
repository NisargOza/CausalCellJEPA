# Run fresh-process bounded CUDA checkpoint/resume gates for all anchored candidates.
import json
import os
import subprocess
import sys
...
assert _git_state()["dirty"] is False, "CUDA anchored smoke requires a clean commit"


def run_phase(name, phase):
    assert torch.cuda.is_available(), "CUDA anchored smoke requires an available GPU"
...
phase = os.environ.get("CAUSALCELLJEPA_ANCHORED_CUDA_PHASE")
candidate = os.environ.get("CAUSALCELLJEPA_ANCHORED_CUDA_CANDIDATE")
if phase:
    ...
    for name in configs:
        ...
        for child_phase in ("checkpoint", "resume"):
            ...
            environment["CAUSALCELLJEPA_ANCHORED_CUDA_PHASE"] = child_phase
            environment["CAUSALCELLJEPA_ANCHORED_CUDA_CANDIDATE"] = name
