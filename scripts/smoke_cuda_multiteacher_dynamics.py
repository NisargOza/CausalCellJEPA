# Run fresh-process bounded CUDA checkpoint/resume gates for both multimodal candidates.
import json
import os
import subprocess
import sys
...
assert _git_state()["dirty"] is False, "CUDA multimodal smoke requires a clean commit"


def run_phase(name, phase):
    assert torch.cuda.is_available(), "CUDA multimodal smoke requires an available GPU"
...
phase = os.environ.get("CAUSALCELLJEPA_MULTITEACHER_CUDA_PHASE")
candidate = os.environ.get("CAUSALCELLJEPA_MULTITEACHER_CUDA_CANDIDATE")
if phase:
    ...
    for name in configs:
        ...
        for child_phase in ("checkpoint", "resume"):
            ...
            environment["CAUSALCELLJEPA_MULTITEACHER_CUDA_PHASE"] = child_phase
            environment["CAUSALCELLJEPA_MULTITEACHER_CUDA_CANDIDATE"] = name
